...
# 构建12x12的转换矩阵T
    ...
    def ElementStress(self, stress, displacement):
        ...
        t_avg = (t1 + t2 + t3 + t4)/4
        J = 2*(b-t_avg)*(h-t_avg)*t_avg
        Qy = (b*h**2)/8 - ((b-t3-t4)*(h-t1-t2)**2)/8
        Qz = (h*b**2)/8 - ((h-t1-t2)*(b-t3-t4)**2)/8
...
# 旋转矩阵R（3x3）
    ...
    # More accurate moment calculation using beam stiffness relations
        M_y1 = (2*E*Iyy/L) * (2*u_local[4] + u_local[10] - (3/L)*(u_local[2] - u_local[8]))
        M_y2 = (2*E*Iyy/L) * (u_local[4] + 2*u_local[10] - (3/L)*(u_local[2] - u_local[8]))
    
        M_z1 = (2*E*Izz/L) * (2*u_local[5] + u_local[11] + (3/L)*(u_local[1] - u_local[7]))
        M_z2 = (2*E*Izz/L) * (u_local[5] + 2*u_local[11] + (3/L)*(u_local[1] - u_local[7]))
    
    # Use average moment for stress calculation
        M_y = M_y1 if abs(M_y1) > abs(M_y2) else M_y2
        M_z = M_z1 if abs(M_z1) > abs(M_z2) else M_z2
